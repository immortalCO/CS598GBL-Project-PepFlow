...
if __name__ == '__main__':
    ...
    parser.add_argument('--local-rank', '--local_rank', dest='local_rank', type=int, default=None, help='Local rank. Necessary for using the torch.distributed.launch utility.')
    ...
    try:
        for it in range(it_first, config.train.max_iters + 1):
            ...
            #     avg_val_loss = validate(it)
                # if not args.debug:
            if it % config.train.val_freq == 0 and local_rank == 0 and (not args.debug):
    ...
    # except RuntimeError:
    #     logger.info('It seems that peers have been terminated.')
    #     distrib.destroy_process_group()
