...
# Keep allocation behavior consistent with training runs.
...
def main():
    ...
    ckpts = [
        ("official_model1", "./ckpts/model1.pt"),
        (
            "option2_grpo_iter1000",
            "./outputs/option2_grpo_seqonly_train4g_learn_angle_2026_05_01__07_30_53/checkpoints/1000.pt",
        ),
    ]
    ...
    print(f"[info] device={device} dataset_len={len(dataset)} num_steps={num_steps} num_samples={num_samples}", flush=True)
    ...
    out_path = os.path.join(out_dir, "quick_eval_option2_vs_official.json")
